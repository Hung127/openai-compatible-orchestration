...
class LLM7Provider(IProvider):
    ...
    def generate_completion(self, request: InternalRequest) -> InternalResponse:
        ...
        for i, j in enumerate(chat_completion["choices"]):
            if j["index"] is None:
                j["index"] = i

        openai_response = OpenAIResponse.model_validate(chat_completion)

        return self.out_converter.to_internal_response(
            openai_response, provider="llm7"
        )
